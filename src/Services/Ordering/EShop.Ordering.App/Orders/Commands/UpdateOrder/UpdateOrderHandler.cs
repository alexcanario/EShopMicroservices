...
public class UpdateOrderHandler(IOrderingDbContext ctx) : ICommandHandler<UpdateOrderCommand, UpdateOrderResult>
{
	...
	private static void UpdateOrderWithNewValues(Order order, OrderDto orderDto)
	{
		...
		var updatedPayment = Payment.Of(orderDto.Payment.CardName, orderDto.Payment.CardNumber, orderDto.Payment.ExpirationDate, orderDto.Payment.Cvv, orderDto.Payment.PaymentMethod.ToString());
